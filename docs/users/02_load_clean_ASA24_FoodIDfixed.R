...
# Load the formatted Items file with SampleID added.
  items_f_id <- read.table("VVKAJ_Items_f_id.txt", sep="\t", header=T, quote="", colClasses="character")
...
# than items_raw because a new column of SampleID has been added.
  dim(items_f_id) # 779 133
  
    ####
    dim(items_raw)  # 779 130
    ####
...
# Load the output for further processing.
  items_f_id_s <- read.table("VVKAJ_Items_f_id_s.txt", header=T, sep="\t", quote="", colClasses="character")
    head(items_f_id_s)  #FoodID has .0.
...
# ===============================================================================================================
  ...
  ####
  # added quote="" in GenerateTotals funciton, too.
  ####
