...
### QC for females  
  ...
  # Save as a .txt file.
  write.table(QCtotals_MF, "Total_D12_FC_QC_mean_QC_demo_MF.txt", sep="\t", quote=F, row.names=F)
  
  QCtotals_MF <- read.delim(  "Total_D12_FC_QC_mean_QC_demo_MF.txt")  
  
  Comb <- read.delim("Total_D12_FC_QC_mean_QC_demo.txt")  
  MF <- read.delim(  "Total_D12_FC_QC_mean_QC_demo_MF.txt")  
  
  head(Comb[, c("SEQN", "RIAGENDR", "GRMS", "KCAL")])
  head(MF[,   c("SEQN", "RIAGENDR", "GRMS", "KCAL")])
  head(MF)

  
  
# # Define the input data.  This dataframe will be modified after each filter.
#   QCtotals <- meantotal12d_demo
#   
# # Flag if KCAL is <600 or >5700 --> ask remove or not --> if yes, remove those rows
