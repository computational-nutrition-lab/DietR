...
We can view summary statistics of either the individual food data or the totals data. First, let us take a look at items data. This section is not intended to be a complete guide to analysis, but rather to give you some ideas for how to explore this data. 
...
The first six rows of VVKAJ_Items_f_id_s_m_summ.txt looks like this. Summary statistics of some variables are not useful such as RecallNo and RecallAttempt, as they are not to be treated as continuous  variables.
```{r VVKAJ_Food_summ_txt, echo=FALSE}
VVKAJ_Items_f_id_s_m_summ <- read.table("VVKAJ_Items_f_id_s_m_summ.txt", sep="\t", header=T)
head(VVKAJ_Items_f_id_s_m_summ)
```
...
The first six rows of VVKAJ_Tot_m_QCed_summ.txt looks like this.
```{r VVKAJ_Tot_summ_txt, echo=FALSE}
VVKAJ_Tot_m_QCed_summ <- read.table("VVKAJ_Tot_m_QCed_summ.txt", sep="\t", header=T)
head(VVKAJ_Tot_m_QCed_summ)
```
...
Prepare your totals dataset for line plot - insert NA to missing combinations of `UserName` and `RecallNo` (day), and separate rows into NAs and no NAs.
```{r}
...
Plot points and lines separately. Ensure to specify your "x" and "y" twice. The `geom_line` function only connects the data of individuals with all days of data.
```{r}
lineplot_1 <- ggplot() + 
