# ===============================================================================================================
# Perform PCA analysis with NHANES males in their 60s data.
# Version 1
...
# Your input data should be a data frame with variables with non-zero variance. 
  pca_input <- read.table("males60s_QCtotal_d_glu_body_meta_demo_Nut_rv.txt", 
                          sep="\t", header=T)
...
# Specify the directory (folder) to save the results.
  res_dir_Nut = "males60s_Nut_PCA" 

# Specify the prefix of filenames to be saved. 
  res_prefix_Nut = "males60s_Nut"
...
# Input is your items/totals input file before any prep for clustering, from which you derived the input for the PCA.
  SaveInputAndPCs(input="QCtotal_d_glu_body_meta_demo_males60s.txt", pca.results = scaled_pca, 
                  out.dir= res_dir_Nut, out.prefix= res_prefix_Nut)
...
# Load the input & PC info.
  Nut_PCs <- read.table("males60s_Nut_PCA/males60s_Nut_PCs.txt", sep="\t", header=T)
...
# Save as a .pdf.
  ggsave("males60s_Nut_PCA/males60s_Nut_PCA_by_GLU_index_PC12_ell.pdf", ell, 
         device="pdf", width=7, height=6.5)

# ---------------------------------------------------------------------------------------------------------------
# Load the glu_3_males60s data. (The original data before filtering variables)
  glu_3_males60s <- read.table("QCtotal_d_glu_body_meta_demo_males60s.txt", 
                               sep="\t", header=T)
  colnames(glu_3_males60s)
  
# Change GLU_index to a factor so that factors will be displayed in order.
  glu_3_males60s$GLU_index <- factor(glu_3_males60s$GLU_index, 
...
# Use the autoplot function. Specify which PC in the x and y arguments.
  food_Nut_PCA <- autoplot(scaled_pca, x=1, y=2,    
                           ...
                           data = glu_3_males60s,  size= 3 ) +   # data is the original input, not after selecting specific variables.  
  ...
  ggsave("males60s_Nut_PCA/males60s_Nut_PCA_by_GLU_index_PC12.pdf", food_Nut_PCA, 
         device="pdf", width=7, height=6.5)
...
# Your input data should be a data frame with variables with non-zero variance. 
  pca_input <- read.table("males60s_QCtotal_d_glu_body_meta_demo_Cat_rv.txt", 
                          sep="\t", header=T)
...
# Specify the directory (folder) to save the results.
  res_dir_Cat = "males60s_Cat_PCA" 

# Specify the prefix of filenames to be saved. 
  res_prefix_Cat = "males60s_Cat"
...
# Load the input & PC info.
  Cat_PCs <- read.table("males60s_Cat_PCA/males60s_Cat_PCs.txt", sep="\t", header=T)
...
# Save as a .pdf.
  ggsave("males60s_Cat_PCA/males60s_Cat_PCA_by_GLU_index_PC12_ell.pdf", ell, 
        device="pdf", width=7, height=6.5)
...
# Use the autoplot function. Specify which PC in the x and y arguments.
  food_Cat_PCA <- autoplot(scaled_pca, x=1, y=2,  
                           loadings=T, loadings.label=T, loadings.colour = 'grey50',  # loadings.label=T if want to see it
                           data = glu_3_males60s,  size= 3 ) +            # The original data before filtering.
    geom_point(size = 3, alpha = 1, na.rm = T, shape = 21,  aes(fill= GLU_index)) +
    theme_bw(base_size = 12) + theme(aspect.ratio = 1) +  
    theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
    scale_fill_manual( values= c("steelblue3", "yellow", "hotpink")) 
  ...
  ggsave("males60s_Cat_PCA/males60s_Cat_PCA_by_GLU_index_PC12.pdf", food_Cat_PCA, 
         device="pdf", width=7, height=6.5)
