...
# Your input data should be a data frame with variables with non-zero variance. 
  pca_input <- read.table("males60to79_QCtotal_d_ga_body_meta_glu_comp_2_Nut_rv.txt", 
                          sep="\t", header=T)
...
# Input is your items/totals input file before any prep for clustering, from which you derived the input for the PCA.
  SaveInputAndPCs(input="QCtotal_d_ga_body_meta_glu_comp_2_males60to79.txt", pca.results = scaled_pca, 
                  out.dir= res_dir_Nut, out.prefix= res_prefix_Nut)
...
# Ellipses.
  ell <- 
    ...
    labs(x = paste("PC1 (", round(Nut_var_exp[1, 2]*100, 2), "%)", sep=""),  
         y = paste("PC2 (", round(Nut_var_exp[2, 2]*100, 2), "%)", sep="")) +
    stat_ellipse(data=Nut_PCs, aes(color=GLU_index), level=0.95) +
    scale_color_manual(values = c("steelblue3", "gold3", "hotpink")) # colors for ellipses
  ell
  
# Save as a .pdf.
  ggsave("males60to79_Nut_PCA/males60to79_Nut_PCA_by_GLU_index_PC12_ell.pdf", ell, 
         device="pdf", width=7, height=6.5)

# ---------------------------------------------------------------------------------------------------------------
# Load the totals_males60to79 data. (The original data before filtering variables)
  totals_males60to79 <- read.table("QCtotal_d_ga_body_meta_glu_comp_2_males60to79.txt", 
                               sep="\t", header=T)
  colnames(totals_males60to79)
...
# Use the autoplot function. Specify which PC in the x and y arguments.
  food_Nut_PCA <- autoplot(scaled_pca, x=1, y=2,    
                           ...
                           data = totals_males60to79,  size= 3 ) +   # data is the original input, not after selecting specific variables.  
  ...
  ggsave("males60to79_Nut_PCA/males60to79_Nut_PCA_by_GLU_index_PC12.pdf", food_Nut_PCA, 
         device="pdf", width=7, height=6.5)
...
# Your input data should be a data frame with variables with non-zero variance. 
  pca_input <- read.table("males60to79_QCtotal_d_glu_body_meta_demo_Cat_rv.txt", 
                          sep="\t", header=T)
...
# Combine the input (totals before processing) with all the variables and the PC results. 
# Input is your items/totals input file before any prep for clustering, from which you derived the input for the PCA.
  SaveInputAndPCs(input="QCtotal_d_glu_body_meta_demo_males60to79.txt", pca.results = scaled_pca, 
                  out.dir= res_dir_Cat, out.prefix= res_prefix_Cat)
# [NOTE] Even though the input file has both nutrients (Nut) and food categories (Cat) data,  
# PCA was done with only either Nut or Cat, not both.
...
# Load the input & PC info.
  Cat_PCs <- read.table("males60to79_Cat_PCA/males60to79_Cat_PCs.txt", sep="\t", header=T)
...
# Save as a .pdf.
  ggsave("males60to79_Cat_PCA/males60to79_Cat_PCA_by_GLU_index_PC12_ell.pdf", ell, 
        device="pdf", width=7, height=6.5)

# ---------------------------------------------------------------------------------------------------------------
# Load the totals_males60to79 data. 
  totals_males60to79 <- read.table("QCtotal_d_glu_body_meta_demo_males60to79.txt", 
                               sep="\t", header=T)
...
# Use the autoplot function. Specify which PC in the x and y arguments.
  food_Cat_PCA <- autoplot(scaled_pca, x=1, y=2,  
                           loadings=T, loadings.label=T, loadings.colour = 'grey50',  # loadings.label=T if want to see it
                           data = totals_males60to79,  size= 3 ) +            # The original data before filtering.
    geom_point(size = 3, alpha = 1, na.rm = T, shape = 21,  aes(fill= GLU_index)) +
    theme_bw(base_size = 12) + theme(aspect.ratio = 1) +  
    theme( panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
    scale_fill_manual( values= c("steelblue3", "yellow", "hotpink")) 
  ...
  ggsave("males60to79_Cat_PCA/males60to79_Cat_PCA_by_GLU_index_PC12.pdf", food_Cat_PCA, 
         device="pdf", width=7, height=6.5)
