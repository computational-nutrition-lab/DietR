...
# Load the food file again that was used to build the phyfoods object. 
  # Dehydrated 
  food1 <- read.delim('../Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.food.otu_d_sortedbysample.txt')
  # # Regular
  # food <- read.delim('../Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.food.otu_sortedbysample.txt')   

# SEQN-sorted food
  dim(food1) # 1678 foods x {237 people + 1 taxonomy columns}
  food1[1:3, 1:3]
  food1[1:7, 1:7]
  max(colSums(food1[, -238]))
  max(colSums(food[, -238]))
...
# Load the SEQN and Axis values.
  loaded_leg_w <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_meta_users.txt",
                             sep="\t", header=T)

  # loaded_leg_u <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_UNweighted_meta_users.txt",
  #                            sep="\t", header=T)  
...
# x <- as.data.frame(t(foodgroups_s))
  ...
  write.table(as.data.frame(colSums(x)), 
              "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_d_AmountSums.txt", 
...
# Mark rows that have qvalues < 0.25 with an asterisk in a column called "Significance".  
  ...
  # Careful which one to save!
    # WEIGHTED
    write.table(dat, "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_corr_axes_foods_thr0.05.txt",
                sep="\t", row.names=F, quote=F)  
  
    # UNweighted
    write.table(dat, "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_UNweighted_corr_axes_foods_thr0.05.txt",
                sep="\t", row.names=F, quote=F)  
