...
# Load the food file again that was used to build the phyfoods object. 
  food1 <- read.delim('../Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.food.otu_sortedbysample.txt')   
...
# Load the SEQN and Axis values.
  loaded_leg_w <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_WEIGHTED_meta_users.txt",
                             sep="\t", header=T)  
...
# x <- as.data.frame(t(foodgroups_s))
  ...
  write.table(as.data.frame(colSums(x)), 
              "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_AmountSums.txt", 
...
# Mark rows that have qvalues < 0.25 with an asterisk in a column called "Significance".  
  ...
  # Careful which one to save!
    # WEIGHTED
    write.table(dat, "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_WEIGHTED_corr_axes_foods_thr0.05.txt",
                sep="\t", row.names=F, quote=F)  
  
    # UNweighted
    write.table(dat, "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_UNweighted_corr_axes_foods_thr0.05.txt",
                sep="\t", row.names=F, quote=F)  
