...
# Save. This will be the input for the following procedures.
  write.table(sel.food.record, "Food_D12_FC_QC_demo_QCed_Leg.txt", 
...
# Keep only the foods reported in your study. This is to make data compatible to create a food tree.
  FilterDbByDietRecords(food_database_fn = "../../Food_tree_eg/NHANESDatabase.txt",
                        food_records_fn  = "Food_D12_FC_QC_demo_QCed_Leg.txt",  # output of filtering above.
                        output_fn =        "Food_D12_FC_QC_demo_QCed_Leg_red.txt")
...
# NHANES food database. # If there is, those will be written in the output file named xxx_missing.txt.
  check.db(food_database_fn = "../../Food_tree_eg/NHANESDatabase.txt", 
           food_records_fn =  "Food_D12_FC_QC_demo_QCed_Leg_red.txt",
           output_fn =        "Food_D12_FC_QC_demo_QCed_Leg_red_missing.txt")

# Load the output and check if the output contains anything? 
  mmm = read.table("Food_D12_FC_QC_demo_QCed_Leg_red_missing.txt", sep="\t", header=T)
  head(mmm)
...
# "NodeLabelsMCT.txt" has a list of food levels and names, which comes with the DietR package.
  MakeFoodTree(nodes_fn="../../Food_tree_eg/NodeLabelsMCT.txt", 
               addl_foods_fn = NULL,
               num.levels = 3,
               food_database_fn =            "Food_D12_FC_QC_demo_QCed_Leg_red.txt",  
               output_tree_fn =     "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.nwk", 
               output_taxonomy_fn = "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.tax.txt"
  )
...
# For the food_records_fn argument, you need to supply 'sel.food.records' file that have 'FoodAmt' column.     
  MakeFoodOtu(food_records_fn=  "Food_D12_FC_QC_demo_QCed_Leg.txt",   
              ...
              food_taxonomy_fn= "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.tax.txt",       # Your taxonomy file produced by MakeFoodTree.
              output_fn =       "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.food.otu.txt")  # Output otu file to be saved.
  
# Make a food otu table with data in grams of fiber per food.
  MakeFiberOtu(food_records_fn=  "Food_D12_FC_QC_demo_QCed_Leg.txt", 
               food_record_id=   "SEQN", 
               food_taxonomy_fn= "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.tax.txt", 
               output_fn=        "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.fiber.otu.txt")
  
# Make a food otu table as dehydrated grams per kcal.
  MakeDhydrtOtu(food_records_fn=  "Food_D12_FC_QC_demo_QCed_Leg.txt", 
                food_record_id =  "SEQN", 
                food_taxonomy_fn= "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.tax.txt", 
                output_fn =       "Foodtree/Food_D12_FC_QC_demo_QCed_Leg_red_3Lv.dhydrt.otu.txt")  
