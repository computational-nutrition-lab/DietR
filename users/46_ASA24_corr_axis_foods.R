...
# x <- as.data.frame(t(foodgroups_s))
  ...
  write.table(as.data.frame(colSums(x)), 
              "VVKAJ_Items_f_id_s_m_QCed_4Lv_AmountSums.txt", 
...
# Mark rows that have qvalues < 0.25 with an asterisk in a column called "Significance".  
  ...
  dat[order(dat$pval), ][1:10, ]
  
  # Careful which one to save!
    # WEIGHTED         
    write.table(dat, "VVKAJ_Items_f_id_s_m_QCed_4Lv_ord_WEIGHTED_corr_axes_foods_thr0.05.txt",
...
# Select significant food items.
  ...
  head(dat_sig[order(dat_sig$qval), ])
...
# WEIGHTED
  write.table(dat_sig[order(dat_sig$qval), ], 
              "VVKAJ_Items_f_id_s_m_QCed_4Lv_ord_WEIGHTED_corr_axes_foods_sig.txt",
              sep="\t", row.names=F, quote=F)  
  
# UNweighted
  write.table(dat_sig[order(dat_sig$qval), ], 
              "VVKAJ_Items_f_id_s_m_QCed_4Lv_ord_UNweighted_corr_axes_foods_sig.txt",
              sep="\t", row.names=F, quote=F)  
