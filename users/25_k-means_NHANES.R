...
# Your input data should be a data frame with variables with non-zero variance.
  nut_kmeansinput <- read.table("males60to79_QCtotal_d_ga_body_meta_glu_comp_2_Nut_rv.txt", 
                                sep="\t", header=T)
...
# Your input data should be a data frame with variables with non-zero variance. 
  cat_kmeansinput <- read.table("males60to79_QCtotal_d_ga_body_meta_glu_comp_2_Cat_rv.txt", 
                                sep="\t", header=T)
...
# distinct peak, but the silhouette method gave a peak at K=2. 

# With specific K values in mind, perform k-means analysis with one specified K.
# Also, change the file name to be saved as a PDF. 
  OneK(myK= 2, out.dir= res_dir_cat, out.fn = "males60to79_Cat_K2")
  oneKplot
