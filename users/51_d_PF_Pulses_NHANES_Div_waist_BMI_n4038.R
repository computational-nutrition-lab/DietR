...
# ===============================================================================================================
  ...
  # FIBE doesn't have effect (p=0.08529). Remove FIBE from model.
  
  emmeans::emmeans(lm.BMI.agf, pairwise ~ DivGroup )
  # DivNA-Div2=  1.9880, p <.0001
  # emmeans look OK, but need to check whether I need FIBE or not.

  aggregate(df$BMXBMI, list(df$DivGroup), FUN=mean) 
  # emmeans are close to these plain averages. 

# ===============================================================================================================
# ancova - BMI ~ DivGroup + Age + Gender
# ===============================================================================================================
  lm.BMI.ag <-    lm( BMXBMI ~ DivGroup + RIDAGEYR + Gender, data=df)
  emmeans::emmeans(lm.BMI.ag, pairwise ~ DivGroup )
  # DivNA-Div2= 2.288, p<.0001. The difference is larger now.
  
  car::Anova(lm.BMI.ag, type="III")
  # all terms are significant.
  
  # compare models with and without fiber.
  anova(aov(lm.BMI.agf), aov(lm.BMI.ag))
...
# ancova - BMI ~ DivGroup + BMXWAIST + Age + Gender + FIBE.
# ===============================================================================================================
  lm.BMI.agfw <-    lm( BMXBMI ~ DivGroup + RIDAGEYR + Gender + FIBE + BMXWAIST, data=df)

  car::Anova(lm.BMI.agfw, type="III")
  # DivGroup, FIBE are not significant anymore! BMXWAIST has a huge sum of squares and F value.
  
  emmeans::emmeans(lm.BMI.agfw, pairwise ~ DivGroup )
  # DivNA-Div2=  -0.111, p=0.8855.
  # The emmeans are almost the same between DivGroups, 29.40-29.62.
