...
# Input is your items/totals input file with no missing data in your variables of interest.
  SaveInputAndPCs(input="QCtotal_d_ga_body_meta_glu_comp_2_males60to79_c_Nut.txt", pca.results = scaled_pca, 
                  out.dir= res_dir_Nut, out.prefix= res_prefix_Nut)
...
# Combine the input with all the variables and the PC results. 
# Input is your items/totals input file with no missing data in your variables of interest.
  SaveInputAndPCs(input="QCtotal_d_ga_body_meta_glu_comp_2_males60to79_c_Cat.txt", pca.results = scaled_pca, 
                  out.dir= res_dir_Cat, out.prefix= res_prefix_Cat)
# [NOTE] Even though the input file has both nutrients (Nut) and food categories (Cat) data,  
# PCA was done with only either Nut or Cat, not both.
...
# Use the autoplot function. Specify which PC in the x and y arguments.
  ...
  ggsave("males60to79_Cat_PCA/males60to79_c_Cat_PCA_by_GLU_index_PC12.png", 
         food_Cat_PCA, device="png", width=7, height=6.5)
