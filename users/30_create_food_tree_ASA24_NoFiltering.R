# ===============================================================================================================
# Generate a food tree from ASA24 data without using FilterDBByDiet.
# Version 1
...
# Prepare your dataset for generating a foodtree.
# This function takes out six columns ("DRXFCSD", "Main.food.description", "Old.Main.food.description", 
# "ModCode", "FoodID") from your dataset and save them into a new file. 
# ftc means food tree columns.
  
  items <- read.delim("VVKAJ_Items_f_id_s_m_QCed.txt")
  which(colnames(items) == "DRXFCSD")
  which(colnames(items) == "FoodID")
  
  PrepFoodtreeInput(food_records_fn = "VVKAJ_Items_f_id_s_m_QCed.txt",
                    out_fn          = "VVKAJ_Items_f_id_s_m_QCed_ftc.txt")
...
####  
# Maybe I don't even need the   PrepFoodtreeInput  function and can just put the items file directly into
# the MakeFoodTree function...? 
  MakeFoodTree(nodes_fn="../Food_tree_eg/NodeLabelsMCT.txt", 
  ...
  # This went through!!! And the foodtree can be visualized. 
####  
