...
### Food -----------------------------------------------------------
  ...
  food_raw <- read.delim("Foodtree/Food_D12_FC_QC_demo_QCed_leg_3Lv.food.otu.txt", row.names=1)
  ...
  write.table(food, "Foodtree/Food_D12_FC_QC_demo_QCed_leg_3Lv.food.otu_sortedbysample.txt",
...
### Sample -----------------------------------------------------------
  ...
  food.record_leg <- read.delim("Food_D12_FC_QC_demo_QCed_Leg.txt")
  
  # From demog_leg, select only the 703 individuals who consumed legume-containing foods which are in food.record_leg.  
  demog_leg_red <- demog_leg[demog_leg$SEQN %in% unique(food.record_leg$SEQN),  ] 
  ...
  foodtree <- read_tree("Foodtree/Food_D12_FC_QC_demo_QCed_leg_3Lv.nwk")
...
# Make a phyloseq object with OTU, TAX, samples, and foodtree by using the phyloseq function.
  ...
  # View the content of the phyfoods object.  
  phyfoods
  # It should say:
  # phyloseq-class experiment-level object
  # otu_table()   OTU Table:         [ 58 taxa and 703 samples ]
  # sample_data() Sample Data:       [ 703 samples by 48 sample variables ]
  # tax_table()   Taxonomy Table:    [ 58 taxa by 5 taxonomic ranks ]
  # phy_tree()    Phylogenetic Tree: [ 58 tips and 21 internal nodes ]
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_w, 
        output.fn="Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_WEIGHTED_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_w, number.of.axes= 10, meta.data= demog_leg_red, 
                              output.fn= "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_WEIGHTED_meta_users.txt")
...
# Load the eigenvalues as a vector.
  eigen_loaded <- read.table("Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_WEIGHTED_eigen.txt", header=T)
...
# dot.colors are for datapoints, and ellipses.colors are for ellipses outlines. 
  PlotAxis1to4ByFactor(axis.meta.df    = loaded_leg_w, 
                       ...
                       out.prefix = "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_WEIGHTED"
                       )
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_u, 
        output.fn="Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_UNweighted_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_u, number.of.axes= 10, meta.data= demog_leg_red, 
                              output.fn= "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_UNweighted_meta_users.txt")
...
# [NOTE} For UNweighted results, change the input, eigen vectors, and prefix names accordingly. 
  PlotAxis1to4ByFactor(axis.meta.df    = loaded_leg_u, 
                       ...
                       ellipses.cflevel = 0.95,
                       out.prefix = "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_UNweighted"
  )
...
# Generate and save an WEIGHTED unifrac distance matrix of "Samples". 
  WeightedUnifracDis(input.phyloseq.obj = phyfoods, 
                     output.fn = "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_WEIGHTED_uni_dis.txt")        

# ---------------------------------------------------------------------------------------------------------------
# Generate and save an UNweighted unifrac distance matrix of "Samples". 
  UnweightedUnifracDis(input.phyloseq.obj = phyfoods, 
                       output.fn = "Food_D12_FC_QC_demo_QCed_leg_3Lv_ord_UNweighted_uni_dis.txt")        
...
#### NOTES ON 02/08/2023.
# adonis is significant, but the Y-shaped biplots are not too good-looking. 
# try 
