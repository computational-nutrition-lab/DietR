...
# Sort individuals (rows) in order. **IMPORTANT!** 
  ...
  dim(food3_s) # 703 people x 58 foods
# food3_s is x. 
...
# loaded_leg_u has vectors (Axis values) of each SEQN.
    ...
    CheckExpression <- function(dataset, starting.str){
      cols <- grepl(paste0("^", starting.str),colnames(dataset), ignore.case=F)
      subset(dataset, select=colnames(dataset)[cols])
    }
    
    # pick up only columns whose names start with "Axis"
    loaded_leg_u_Axisonly <- CheckExpression(dataset = loaded_leg_u, starting.str = "Axis.")
    # Only the Axis values and the rownames (SEQN) were selected.
...
# Mark rows that have qvalues < 0.25 with an asterisk in a column called "Significance".  
  ...
  # Careful which one to save!
    # WEIGHTED
    write.table(dat, "Food_D12_FC_QC_demo_QCed_leg_ftc_3Lv_ord_WEIGHTED_corr_axes_foods.txt",
                sep="\t", row.names=F, quote=F)  
  
    # UNweighted
    write.table(dat, "Food_D12_FC_QC_demo_QCed_leg_ftc_3Lv_ord_UNweighted_corr_axes_foods.txt",
                sep="\t", row.names=F, quote=F)  
...
# Select Axis 1 rows   
  ...
  head(dat_1[order(dat_1$qval), ], 15)
  head(dat_1[order(dat_1$qval), ], 30)
