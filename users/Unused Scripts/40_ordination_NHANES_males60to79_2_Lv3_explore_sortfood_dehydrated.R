...
# Food 
  ...
  # food_raw <- read.delim("Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.food.otu.txt", row.names=1)
  # dehydrated OTU table.
  food_raw <- read.delim("Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.dhydrt.otu.txt", row.names=1)
  ...
  write.table(food, "Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.food.otu_d_sortedbysample.txt",
...
# Taxonomy (tax)
  ...
  tax <- read.delim("Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.tax.txt")
...
# Food tree
  ...
  foodtree <- read_tree("Foodtree/Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv.nwk")
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_w, 
        output.fn="Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_w, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_meta_users.txt")
  
# Load the output again for plotting.
  loaded_glu_w <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_meta_users.txt",
                             sep="\t", header=T)
...
# Load the eigenvalues as a vector.
  eigen_loaded <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED_eigen.txt", header=T)
...
# dot.colors are for datapoints, and ellipses.colors are for ellipses outlines. 
  PlotAxis1to4ByFactor(axis.meta.df    = loaded_glu_w, 
                       ...
                       out.prefix = "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_WEIGHTED"
                       )
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_u, 
        output.fn="Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_UNweighted_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_u, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_UNweighted_meta_users.txt")
  
# Load the output again for plotting.
  loaded_glu_u <- read.table("Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_UNweighted_meta_users.txt",
                             sep="\t", header=T)  
...
# [NOTE} For UNweighted results, change the input, eigen vectors, and prefix names accordingly. 
  PlotAxis1to4ByFactor(axis.meta.df    = loaded_glu_u, 
                       ...
                       ellipses.colors = c("steelblue3", "gold3", "hotpink"),  
                       ellipses.cflevel = 0.95,
                       out.prefix = "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_d_UNweighted"
  )
...
# Generate and save an WEIGHTED unifrac distance matrix of "Samples". 
  WeightedUnifracDis(input.phyloseq.obj = phyfoods, 
                     output.fn = "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_WEIGHTED_uni_dis.txt")        

# ---------------------------------------------------------------------------------------------------------------
# Generate and save an UNweighted unifrac distance matrix of "Samples". 
  UnweightedUnifracDis(input.phyloseq.obj = phyfoods, 
                       output.fn = "Food_D12_FC_QC_demo_QCed_males60to79_ftc_3Lv_ord_UNweighted_uni_dis.txt")        
