...
# Save. This will be the input for the following procedures.
  ...
  write.table(sel.food.record, "Food_D12_FC_cc_f_males60to79.txt", 
              sep="\t", row.names=F, quote=F) 
  head(sel.food.record)
...
# Check if there is any food item reported by people but are missing in the database. 
  check.db(food_database_fn = "../../Food_tree_eg/NHANESDatabase.txt", 
           ...
           output_fn =        "Food_D12_FC_cc_f_males60to79_red_missing.txt")

# Load the output and check if the output contains anything? 
  mmm = read.table("Food_D12_FC_cc_f_males60to79_red_missing.txt", sep="\t", header=T)
  head(mmm)
...
# "NodeLabelsMCT.txt" has a list of food levels and names, which comes with this package.
  MakeFoodTree(nodes_fn="../../Food_tree_eg/NodeLabelsMCT.txt", 
               addl_foods_fn = NULL,
               num.levels = 3,
               food_database_fn =   "Food_D12_FC_cc_f_males60to79_red.txt",  
               output_tree_fn =     "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.nwk", 
               output_taxonomy_fn = "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.taxonomy.txt"
  ) 
...
# Make the standard food otu table with data in gram weights of food.
  MakeFoodOtu(food_records_fn=  "Food_D12_FC_cc_f_males60to79.txt",  # need to supply the original data that have 'FoodAmt' before FilterDBByDietRecords.  
              ...
              food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.taxonomy.txt",  # Your taxonomy file produced by MakeFoodTree.
              output_fn =       "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.food.otu.txt")  # Output otu file to be saved.

# Make a food otu table with data in grams of fiber per food
  MakeFiberOtu(food_records_fn=  "Food_D12_FC_cc_f_males60to79.txt", 
               food_record_id=   "SEQN", 
               food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.taxonomy.txt", 
               output_fn=        "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.fiber.otu.txt")

# Make a food otu table as dehydrated grams per kcal
  MakeDhydrtOtu(food_records_fn=  "Food_D12_FC_cc_f_males60to79.txt", 
                food_record_id =  "SEQN", 
                food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.taxonomy.txt", 
                output_fn =       "Foodtree/Food_D12_FC_cc_f_males60to79_red_Lv3.dhydrt.otu.txt")  
