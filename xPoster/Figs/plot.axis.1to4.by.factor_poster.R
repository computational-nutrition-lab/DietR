...
PlotAxis1to4ByFactor <- function(axis.meta.df, 
  ...
  p12 <- ggplot(axis.meta.df.factor_Axis1to4, aes(x=Axis.1, y=Axis.2, color=Factor_to_color )) +
    geom_point(shape=21, aes(fill= Factor_to_color), size=4, color="black") + 
    scale_fill_manual( values= dot.colors)  + theme_bw(base_size = 16) +
  ...
  # Save your plot. 
  ggsave( paste(out.prefix, "_Axis12.png", sep=""), 
          p12, device="png", width=7, height=6.5, units="in") 
  ...
  # Save your plot. 
  ggsave( paste(out.prefix, "_Axis12_ellipses.png", sep=""), 
          p12_ellipses, device="png", width=7, height=6.5, units="in") 
